"""
Defines all the house-hold objects pertaining to media. Includes the enums required,
and the basic objects that can be mapped directly from the API.

The code present in this class relates more to defining the structure and the overall
interface of these objects, as opposed to defining how the data is to be internally
handled and used.
"""
...
class MediaStats(BaseObject):
    def __init__(
        ...
        """
        Contains media statistics.

        Args:
            score_distribution: List containing a distribution of scores.
            status_distribution: List containing distribution of statuses.
        """

        if not all(
            isinstance(x, list) for x in (score_distribution, status_distribution)
        ):
            raise TypeError

        self.scoreDistribution = score_distribution
        self.statusDistribution = status_distribution
    ...
    def initialize(data: Union[str, Dict[Any, Any]]) -> Any:
        ...
        return MediaStats(
            score_distribution=[
                ScoreDistribution.initialize(data)
                for data in final_data.get("scoreDistribution", [])
            ],
            status_distribution=[
                StatusDistribution.initialize(data)
                for data in final_data.get("statusDistribution", [])
            ],
        )
